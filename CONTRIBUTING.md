...
3. Build and test the CLI (from `cli/`):
   ...
   A plain `go build` reports `dev` for `fellowship version`. To stamp a real version, build with:

   ```bash
   go build -ldflags "-X main.version=<version>" -o fellowship ./cmd/fellowship
   ```
...
- Posture: gate hooks (`gate-guard`, `gate-submit`, `gate-prereq`, `file-track`, `agent-track`) fail *closed* so enforcement can't be silently skipped, but only where blocking is actually the safe answer. Precisely:
  ...
  - **Out-of-date store** — the store was written by an older binary. Hooks never migrate: running the schema ladder from a hook would have every tool call racing to rewrite the store it is reading. Gate hooks **block** (exit 2, "run `fellowship status` to upgrade it"); every non-hook command migrates on open. The one exception: `gate-guard` gates `Bash`, so it lets a read-only fellowship command through — otherwise the block would deny its own remedy and freeze every session in the repo after any binary upgrade. The allowance is the escape allowlist, not `fellowship init`: `init` also *resets* an existing quest row, so allowing it would hand a gate-blocked teammate its own release the first time a binary upgrade made the store stale — the one moment gate-guard cannot read the gate flag to refuse it. Nothing may be chained onto the allowance (shell metacharacters disqualify it), and it applies only to a stale store, never to a missing or zero-byte one. The two hooks that only decide (`gate-guard`, `worktree-guard`) open the store read-only.
  ...
  - **Lead commands are refused inside a quest worktree.** `gate-guard` runs only where a quest row resolved, so any `fellowship state ...` it sees is a teammate reaching for the lead's own command set — above all `state init`, which records which session is the lead. A teammate that ran it would become the lead for `worktree-guard` and for the phase rule below, and lock the real lead out of its own tree. The whole Bash command line is scanned, not just its first word: operators (`&&`, `;`, `|`, `&`), command substitution and subshells (`$(...)`, backticks, `(...)` — which run inside double quotes as well as outside), `sh -c "..."` / `eval` payloads, and any path to the binary (including the `fellowship.sh` wrapper) are all followed. Quoting is respected, so a command merely *named* inside an argument (`git commit -m "fellowship state init"`) is not an invocation. *Every* invocation on the line is judged, not just the first one found: `init --phase <the phase the quest is already in>` moves nothing and is waved through, so stopping at it would hand a teammate whatever it chained behind it.
  - **The lead row is written once.** `state init` records the lead only when none is recorded; re-running it leaves the existing lead alone and says so. Re-recording is `state init --claim-lead` — on its own, or alongside `--name` when a re-init should also take the lead — which runs only from the main working tree and refuses a session that is recorded against a quest. That data comes from `quest_state.session_id`, written by `fellowship init` from `CLAUDE_CODE_SESSION_ID` — if teammate sessions run without that variable, the store has no id to match and this backstop is inert; the gate-guard rule above is what carries the case.
...
- **Who may write in the main tree.** The main working tree is the lead's own workspace, so "a source write in the main tree during an active fellowship" is not by itself a mis-placement — `worktree-guard` has to tell the lead apart from a teammate that was dropped into the main tree. Nothing in the git topology does that (both resolve to the same top-level), so the guard identifies the *session*:
  ...
  - At hook time the guard compares the payload's `session_id` with the recorded lead, in this order: **(1)** payload session id == recorded id AND the payload carries no `agent_id` → the lead, allow; **(2)** the session's git top-level is a registered quest worktree → a teammate writing into the main tree, block (this one needs no session ids); **(3)** both ids are known and differ, or the payload carries an `agent_id` — a subagent is never the lead → a session that is not the lead, block; **(4)** otherwise the writer cannot be identified — no recorded lead, or no id in the payload — and the fail-open backstop allows.
  ...
  - Consequence to know: if the lead's session id changes mid-fellowship (a brand-new session in the main tree rather than a resumed one), rule 3 blocks it. `fellowship state init --claim-lead` re-records this session as the lead and changes nothing else — it runs only from the main working tree, refuses a session recorded against a quest, and is itself refused by gate-guard inside a quest worktree, so the teammates the lead row exists to identify cannot use it to name themselves; any `fellowship` command run in the main tree from a non-lead session prints one line saying so. `state init` with no `CLAUDE_CODE_SESSION_ID` in the environment records an anonymous lead and warns, since the guard then falls back to rule 2.
- The wrapper (`fellowship.sh`) applies that same posture to distribution itself: it never execs the `fellowship` binary directly from `hooks.json`, and it never installs it — only the `SessionStart` hook (`ensure-binary.sh`) does that, so a hook invocation can't trigger a network download on the critical path of the tool call it's guarding. If the binary isn't installed when a gate hook runs, every gate hook (`gate-guard`, `gate-submit`, `gate-prereq`, `file-track`, `agent-track`) blocks immediately (exit 2, with a message on stderr pointing at `ensure-binary.sh`) rather than silently letting the tool call through; `worktree-guard` alone exits 0 in that case, consistent with its fail-open backstop posture. `ensure-binary.sh`'s own install lock (a mkdir-based lock in the install directory) records its holder's PID and acquisition time, so a session killed mid-install can't wedge every future install: a contending session reclaims the lock once the recorded PID is no longer alive, or once 120s have passed regardless.

Current hooks: `gate-guard`, `gate-submit`, `gate-prereq`, `file-track`, `agent-track`, `worktree-guard`. Run `fellowship` with no args for the full command reference.

What each of the gate hooks decides, against the four-phase lifecycle (Research → Plan → Implement → Review):

| Hook | Decides |
|---|---|
| `gate-guard` | Blocks a held quest and blocks everything but a read-only escape command while a gate is pending (both read the same allowlist), refuses a non-lead `fellowship init --phase/--plan-skip`, refuses `fellowship complete` unless the quest is in Review with no gate pending (`hooks.CompletionCheck`, the same rule the command applies), refuses any hand-edit of the store, blocks source writes outside the data directory during Research and Plan (`state.IsEarlyPhase`), and refuses a bare `cd` only in the lead's own conversation (a subagent's `cd` would not persist between its Bash calls anyway) |
| `gate-submit` | Detects a `[GATE]` marker, checks the lembas and phase-confirmation prerequisites, and runs the phase transition — including the auto-approve path for a phase named in `gates.autoApprove` |
| `gate-prereq` | Records that `/lembas` ran for this phase |
| `file-track` | Records file touches in the quest history |
| `agent-track` | After a subagent's Bash call, records which quest the `--dir` it named belongs to, so later path-less calls from that same `agent_id` resolve |

Two CLI commands carry enforcement that used to live in hooks: `fellowship phase confirm --dir <worktree> --phase <phase>` records the phase prerequisite only when the phase is valid and equals the quest's current phase, and never changes the phase; `fellowship complete --dir <worktree>` is allowed only in Review with no gate pending, marks the history completed and sets the store status to `completed` — gate-guard also refuses the Bash form of `fellowship complete` when that check fails, so the enforcement is structural, not prompt-only.
