...
- Posture: gate hooks (`gate-guard`, `gate-submit`, `gate-prereq`, `completion-guard`, `metadata-track`, `file-track`) fail *closed* so enforcement can't be silently skipped, but only where blocking is actually the safe answer. Precisely:
  ...
  - **Lead commands are refused inside a quest worktree.** `gate-guard` runs only where a quest row resolved, so any `fellowship state ...` it sees is a teammate reaching for the lead's own command set — above all `state init`, which records which session is the lead. A teammate that ran it would become the lead for `worktree-guard` and for the phase rule below, and lock the real lead out of its own tree. The whole Bash command line is scanned, not just its first word: operators (`&&`, `;`, `|`, `&`), `sh -c "..."` / `eval` payloads, and any path to the binary (including the `fellowship.sh` wrapper) are all followed. Quoting is respected, so a command merely *named* inside an argument (`git commit -m "fellowship state init"`) is not an invocation.
  - **The lead row is written once.** `state init` records the lead only when none is recorded; re-running it leaves the existing lead alone and says so. Re-recording is `state init --claim-lead`, which runs only from the main working tree and refuses a session that is recorded against a quest. That data comes from `quest_state.session_id`, written by `fellowship init` from `CLAUDE_CODE_SESSION_ID` — if teammate sessions run without that variable, the store has no id to match and this backstop is inert; the gate-guard rule above is what carries the case.
