...
<div class="container page">
	...
	<section class="version" id="unreleased">
		...
		<ul class="changes">
			...
			<li>
				<strong>One health classifier, reachable everywhere</strong> &mdash; <code>fellowship eagles</code> and <code>fellowship herald --problems</code> were two separate Go implementations of the same stalled/zombie classification, and palantir carried a third copy in prose driven by unbounded <code>git diff</code>/<code>git status</code> over each worktree. <code>herald.DetectProblems</code> now delegates to eagles' sweep (which gained a <code>struggling</code> classification &mdash; repeated gate rejections in a quest's current phase, independent of its <code>health</code>), and <code>eagles.WriteReport</code> and the <code>.fellowship/eagles-report.json</code> file it wrote (nothing read it) are gone. palantir is now a thin reporter over that one sweep: it runs <code>fellowship eagles --json</code> and <code>fellowship state show --json</code> instead of reconstructing stuck/stalled from task metadata, and reads scope-drift/file-conflict signals from each quest's tome (<code>tome show --json</code>'s <code>files_touched</code>) instead of diffing worktrees itself. Below <code>palantir.minQuests</code> or with <code>palantir.enabled: false</code>, Gandalf runs the same sweep itself after every gate transition and spawn, so health monitoring never depends on an extra agent. <code>/rekindle</code> and <code>/retro</code> read quest state and phase/health the same way instead of shelling <code>gate status --dir &lt;worktree&gt;</code> per quest. <code>state show</code> (always JSON, but had no flag to name that) and <code>company show &lt;name&gt;</code> (table-only before) both accept <code>--json</code> now.
			</li>
			<li>
