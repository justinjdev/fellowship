(function () {
    ...
    if (quest.gate_pending) {
      gateHTML =
        '<div class="gate-actions">' +
          '<button class="btn btn-approve" onclick="window.__approve(\'' + escapeAttr(quest.dir) + '\')">Approve</button>' +
          '<button class="btn btn-reject" onclick="window.__showReject(this)">Reject</button>' +
        "</div>" +
        '<div class="reject-confirm" id="reject-' + escapeAttr(quest.dir) + '">' +
          '<span>Are you sure?</span>' +
          '<button class="btn btn-confirm-reject" onclick="window.__reject(\'' + escapeAttr(quest.dir) + '\')">Confirm Reject</button>' +
          '<button class="btn btn-cancel" onclick="window.__hideReject(this)">Cancel</button>' +
        "</div>";
    }

    card.innerHTML =
      "<h3>" + escapeHTML(quest.name || quest.dir) + "</h3>" +
  ...
  function renderScoutCard(scout) {
    ...
    card.innerHTML =
      "<h3>" + escapeHTML(scout.name || scout.dir) + '</h3>' +
  ...
  function detectChanges(newStatus) {
    ...
    const oldQuests = {};
    (prevStatus.quests || []).forEach(function (q) { oldQuests[q.dir] = q; });

    (newStatus.quests || []).forEach(function (q) {
      const old = oldQuests[q.dir];
      if (!old) {
        addActivity("New quest: " + (q.name || q.dir));
        return;
      }
      if (old.phase !== q.phase) {
        addActivity((q.name || q.dir) + ": " + old.phase + " → " + q.phase);
      }
      if (!old.gate_pending && q.gate_pending) {
        addActivity((q.name || q.dir) + ": gate approval requested");
      }
      if (old.gate_pending && !q.gate_pending) {
        addActivity((q.name || q.dir) + ": gate resolved");
      }
    });
  }
