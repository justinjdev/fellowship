...
1. **The store**: `fellowship state show --json` (names, task descriptions, worktrees, status, groups), `fellowship health --json` (phase, health, gate pending, struggling, last activity), and `fellowship group show <name> --json`.
2. **Gate/complete/report messages**: `[GATE]`, `[COMPLETE]`, and `[REPORT]` messages from teammates provide the most recent context for each quest.
...
| scout-auth-analysis | Scout | Active | █░░ |
...
When groups are defined, group quests by group in the status report. Run `~/.claude/fellowship/bin/fellowship group show <name> --json` for a group's canonical membership and completed count rather than reconstructing it from the store's other commands:
...
Scout phases:
- Active (`█░░`) until its `[REPORT]` message arrives, then Done (`███`)

- Use filled/empty block characters for visual progress
- Pull quest phase from `fellowship health --json`
